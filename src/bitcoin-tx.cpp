...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " aurum-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  aurum-tx [options] <hex-tx> [commands]  Update hex-encoded aurum transaction\n" +
            "or:     aurum-tx [options] -create [commands]   Create hex-encoded aurum transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded aurum transaction
