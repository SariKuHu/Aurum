...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Aurum Crypto Gold address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no aurum: URI
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("aurum:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aurum Crypto Gold.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aurum Crypto Gold (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Aurum Crypto Gold (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Aurum Crypto Gold*.lnk
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("aurum-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a aurum.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Aurum Crypto Gold\n";
        else
            optionFile << strprintf("Name=Aurum Crypto Gold (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(CFArrayRef listSnapshot, LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the aurum app
