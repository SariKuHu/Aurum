...
const char * const BITCOIN_CONF_FILENAME = "aurum.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "aurumd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //aurum-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Aurum Crypto Gold
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Aurum Crypto Gold
    // Mac: ~/Library/Application Support/Aurum Crypto Gold
    // Unix: ~/.aurum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Aurum Crypto Gold";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Aurum Crypto Gold";
#else
    // Unix
