...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fs::exists(config_file_path)) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Aurum Crypto Gold config file:\n"
                                    "rpcuser=username\n"
                                    ...
                                    "\n"
                                    "# Addnodes:\n"
                                    "addnode=node01.aurumcrypto.gold\n"
                                    "addnode=node02.aurumcrypto.gold\n"
                                    "addnode=node03.aurumcrypto.gold\n"
                                    "addnode=node04.aurumcrypto.gold\n"
                                    "\n";
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if aurum is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if aurum is started while in a temporary directory.\n",
